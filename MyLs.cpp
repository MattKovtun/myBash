...
void getFoldersize(string rootFolder, unsigned long long &f_size) {
    ...
    if (exists(folderPath)) {
        ...
        for (fs::directory_iterator dirIte(rootFolder); dirIte != end_itr; ++dirIte) {
            ...
            try {
                if (!is_directory(dirIte->status())) f_size = f_size + file_size(filePath);
...
void printMapLs(const multimap<time_t, fs::path> &m, bool answerL, string sorting, bool answerReverse) {
    ...
    for (int i = 0; i < vector1.size(); i++) {
        if (answerL) {
            ...
            if (fs::is_directory(vector1[i]))
                cout << std::left << std::setfill(' ') << std::setw(40) << "/" + vector1[i].filename().string()
                     << " size: " << std::left << std::setfill(' ') << std::setw(15) << getSize(vector1[i])
                     << " time last modification: " << std::left << std::setfill(' ') << std::setw(30) << std::ctime(&t)
                     << endl;
            else
                cout << std::left << std::setfill(' ') << std::setw(40) << vector1[i].filename().string() << " size: "
                     << std::left << std::setfill(' ') << std::setw(15) << getSize(vector1[i])
                     << " time last modification: " << std::left << std::setfill(' ') << std::setw(30) << std::ctime(&t)
                     << endl;
        } else {
            if (fs::is_directory(vector1[i]))
                cout << std::left << std::setfill(' ') << std::setw(50) << "/" + vector1[i].filename().string() << endl;
